...
// components
import LeactureListHead from '../sections/@dashboard/leacture/LeactureListHead';
import LeactureListToolbar from '../sections/@dashboard/leacture/LeactureListToolbar';
...
// mock
import USERLIST from '../_mock/lecture';
import { Role } from '../env';
import  unPublishedAssignment  from '../_mock/UnPublishedLecture';
...
const TABLE_HEAD = [
  { id: 'name', label: 'Leture List', alignRight: false },
...
export default function                                                                                                                                                                                                                                                                                                       UserPage() {
  ...
  const {getLectureData,lecturesData} = useLectureContext()

   const navigate = useNavigate();
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - USERLIST.length) : 0;

  const filteredUsers = applySortFilter(lecturesData, getComparator(order, orderBy), filterName);
  const filteredAssignment = applySortFilter(unPublishedAssignment, getComparator(order, orderBy), filterName);
  ...
  return (
    <>
      ...
      <Container style={{
        ...
        <Card>
          <LeactureListToolbar numSelected={selected.length} filterName={filterName} onFilterName={handleFilterByName} />
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row,index) => {
                    ...
                    return (
                      <TableRow onClick={()=>{
                        navigate(`/dashboard/perticularlecture/${index}`)
                      }} style={{
                        cursor:"pointer"
              ...
              <Table>
                ...
                <TableBody>
                  {filteredAssignment.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    ...
                    return (
                      <TableRow onClick={()=>{
                        navigate(`/dashboard/perticularlecture/${name}`)
                      }} style={{
                        cursor:"pointer"
                      }} hover key={id} tabIndex={-1} role="checkbox" selected={selectedUser}>
                        <TableCell  padding="checkbox">
